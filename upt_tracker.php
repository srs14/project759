	...
	<div style="width:95%;margin:0 auto;">
			...
			<table cellspacing="0" cellpadding="0" class="manage">
				<tbody>
					<tr>
						...
						<th style="width:32px;" colspan="12"><?php echo $currentYear;?></th>
						<th style="width:32px;" colspan="12"><?php echo $secondYear;?></th>
						<th style="width:32px;" colspan="12"><?php echo $thirdYear;?></th>
						<th style="width:12px;" colspan="3">+</th>
					</tr>
					<tr>
					...
					<?php
						...
						while($tl = mysql_fetch_array($trialsList)){	
							...
							if($tl["start_date"] != '' && $tl["start_date"] !== NULL && $tl["start_date"] != '0000-00-00'){
								...
								$startDateAsTitle = date('M Y', strtotime($tl['start_date']));
								$startDateMonth = date('m', strtotime($tl['start_date']));
								$startDateYear = date('Y', strtotime($tl['start_date']));
							}
							if($tl["end_date"] != '' && $tl["end_date"] !== NULL && $tl["end_date"] != '0000-00-00'){
								$endDate = date('m/y', strtotime($tl['end_date']));
								$endDateAsTitle = date('M Y', strtotime($tl['end_date']));
								$endDateMonth = date('m', strtotime($tl['end_date']));
								$endDateYear = date('Y', strtotime($tl['end_date']));
							}
					...
					?>
						<tr>
							...
							<td class="title " rowspan="1">
								<div class="rowcollapse" style="height: 16px;">
									<a target="_blank" href="<?php echo urldecode($trailLink);?>" style="color:#000000;"><font id="ViewCount_139317"></font><?php echo $tl['brief_title'];?></a>
								</div>
							</td>
							...
							<?php
								$dateTitle = "";
								if(isset($startDateAsTitle) && $startDateAsTitle != "" && isset($endDateAsTitle) && $endDateAsTitle != ""){
									$dateTitle = $startDateAsTitle."-".$endDateAsTitle;
								}else if(isset($startDateAsTitle) && $startDateAsTitle != ""){
									$dateTitle = $startDateAsTitle;
								}else if(isset($endDateAsTitle) && $endDateAsTitle != ""){
									$dateTitle = $endDateAsTitle;
								}
								echo returnGnattChart($startDateMonth,$startDateYear,$endDateMonth,$endDateYear,$currentYear,$secondYear,$thirdYear,$dateTitle,$phaseValues[$tl['phase']]);
							?>
						</tr>
					<?php } ?>
				</tbody>
		...
		<div id="slideout">
			...
			<div class="slideout_inner">
				...
				<tbody><tr><td width="15%"><img alt="Data release" src="images/black-diamond.png"></td><td>Click for data release</td></tr>
				<tr><td><img alt="Data release (new)" src="images/red-diamond.png"></td><td>Click for data release (new)</td></tr>
				<tr><td><img alt="Results pending" src="images/hourglass.png"></td><td>Results pending</td></tr>
				<tr><td><img alt="Milestone result" src="images/black-checkmark.png"></td><td>Click for milestone result</td></tr>
				<tr><td><img alt="Milestone result (new)" src="images/red-checkmark.png"></td><td>Click for milestone result (new)</td></tr>
				<tr><td><img alt="Milestone details" src="images/purple-bar.png"></td><td>Click for milestone details</td></tr>
				<tr><td><img alt="Milestones" src="images/down.png"></td><td>Display milestones</td></tr>
				<tr><td style="padding-right: 1px;" colspan="2">
...
<?php	
	...
	function generateExcelFile($larvolIds,$productTitle){
		...
		while($value = mysql_fetch_array($trialsList)){
			...
			if($value["start_date"] != '' && $value["start_date"] !== NULL && $value["start_date"] != '0000-00-00'){
				...
				$startDateAsTitle = date('M Y', strtotime($value['start_date']));
			}
			if($value["end_date"] != '' && $value["end_date"] !== NULL && $value["end_date"] != '0000-00-00'){
				$endDate = date('m/y', strtotime($value['end_date']));
				$edMonth = date('m', strtotime($value['end_date']));
				$edYear = date('Y', strtotime($value['end_date']));
				$endDateAsTitle = date('M Y', strtotime($value['end_date']));
			}
			$dateAsTitle = "";
			if(isset($startDateAsTitle) && $startDateAsTitle != "" && isset($endDateAsTitle) && $endDateAsTitle != ""){
				$dateAsTitle = $startDateAsTitle."-".$endDateAsTitle;
			}else if(isset($startDateAsTitle) && $startDateAsTitle != ""){
				$dateAsTitle = $startDateAsTitle;
			}else if(isset($endDateAsTitle) && $endDateAsTitle != ""){
				$dateAsTitle = $endDateAsTitle;
			}else{
				$dateAsTitle= "";
			}
			$cellBackgroundColor = $phaseValues[$value['phase']];
			if($value['phase'] == 'N/A' || $value['phase'] == '' || $value['phase'] === NULL){
				$phase = 'N/A';
			}else{
				$phase = str_replace('Phase ', '', trim($value['phase']));
			...
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$i , fix_special_chars($value['intervention_name']));
			...
			$secondYear	= date('Y')+1;
			$thirdYear	= date('Y')+2;
			$chattStartMonth = $stMonth;			
			$chattStartYear = $stYear;			
			$chattEndMonth = $edMonth;			
			$chattEndYear = $edYear;	
			$mergeStart = "";
			$mergeEnd = "";
			$uptoMergeStart = "";
			$afterMergeEnd = "";
			$mergeEndPosition = "";
			if($chattStartMonth == "" || $chattStartYear == "" || $chattEndMonth == "" || $chattEndYear == "" || $chattEndYear < $currentYear || $chattStartYear > $chattEndYear){
				...
				$objPHPExcel->getActiveSheet()->getStyle('M'.$i.':O'.$i)->applyFromArray(array(
					'fill' => array(
						'type'       => PHPExcel_Style_Fill::FILL_SOLID,
						'rotation'   => 0,
						'startcolor' => array('rgb' => $phaseValues[$value['phase']]),
						'endcolor'   => array('rgb' => $phaseValues[$value['phase']]))
				));
			}else{
				$chattStartMonth = ltrim($chattStartMonth,"0");
				$chattEndMonth = ltrim($chattEndMonth,"0");
				if($chattStartYear < $currentYear){
					$mergeStart = 'M';
				}elseif($chattStartYear == $currentYear){
					$objPHPExcel->getActiveSheet()->mergeCells('M'.$i.':O'.$i);
					$mergeStart = 'P';
				}elseif($chattStartYear == $secondYear){
					$objPHPExcel->getActiveSheet()->mergeCells('M'.$i.':O'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('P'.$i.':AA'.$i);
					$mergeStart = 'AB';
				}elseif($chattStartYear == $thirdYear){
					$objPHPExcel->getActiveSheet()->mergeCells('M'.$i.':O'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('P'.$i.':AA'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AB'.$i.':AM'.$i);
					$mergeStart = 'AN';
				...
				if($chattStartYear >= $currentYear){
					$uptoMergeStart = getColspanforExcelExport($mergeStart,$chattStartMonth-1);
				
					$objPHPExcel->getActiveSheet()->setCellValue($mergeStart.$i , '');
					$objPHPExcel->getActiveSheet()->mergeCells($mergeStart.$i.':'.$uptoMergeStart.$i);
				}
				if($chattStartYear < $currentYear){
					$mergeStart = $mergeStart;
				}else{
					$mergeStart = getColspanforExcelExport($mergeStart,$chattStartMonth);
				}
				if($chattStartYear == $chattEndYear){
					$mergeEndPosition = $chattEndMonth-$chattStartMonth+1;
				}elseif($chattStartYear < $currentYear){
					$mergeEndPosition = 12*($chattEndYear-$currentYear)+$chattEndMonth+3;
				}else{
					$mergeEndPosition = 12*($chattEndYear-$chattStartYear)-$chattStartMonth+$chattEndMonth+1;
				}
				$mergeEnd = getColspanforExcelExport($mergeStart,$mergeEndPosition);
				$objPHPExcel->getActiveSheet()->setCellValue($mergeStart.$i , '');
				$objPHPExcel->getActiveSheet()->mergeCells($mergeStart.$i.':'.$mergeEnd.$i);
				$objPHPExcel->getActiveSheet()->getStyle($mergeStart.$i.':'.$mergeEnd.$i)->applyFromArray(array(
					'fill' => array(
						'type'       => PHPExcel_Style_Fill::FILL_SOLID,
						'rotation'   => 0,
						'startcolor' => array('rgb' => $phaseValues[$value['phase']]),
				...
				$afterMergeEnd = ++$mergeEnd;
				
				if($chattEndYear == $currentYear){
					$objPHPExcel->getActiveSheet()->mergeCells($afterMergeEnd.$i.':AA'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AB'.$i.':AM'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AN'.$i.':AY'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AZ'.$i.':BB'.$i);
				}elseif($chattEndYear == $secondYear){
					$objPHPExcel->getActiveSheet()->mergeCells($afterMergeEnd.$i.':AM'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AN'.$i.':AY'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AZ'.$i.':BB'.$i);
				}elseif($chattEndYear == $thirdYear){
					$objPHPExcel->getActiveSheet()->mergeCells($afterMergeEnd.$i.':AY'.$i);
					$objPHPExcel->getActiveSheet()->mergeCells('AZ'.$i.':BB'.$i);
				}else{
					$objPHPExcel->getActiveSheet()->mergeCells($afterMergeEnd.$i.':BB'.$i);
				}
			}
			
			$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':L'.$i)->applyFromArray(changeBackground($i));
			$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':BB'.$i)->applyFromArray(changeOtherStyle($i));
			$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':BB'.$i)->getFont()->setSize(8);
			$i++;
		}
		$objPHPExcel->getActiveSheet()->getStyle('M1:O1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle('P1:AA1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle('AB1:AM1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle('AN1:AY1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->getActiveSheet()->getStyle('AZ1:BB1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	...
	function changeOtherStyle($evenOdd){
		if($evenOdd % 2){
			return array(
				'alignment' => array('horizontal'	=> PHPExcel_Style_Alignment::HORIZONTAL_LEFT,),
				'borders' => array(
	...
	function returnGnattChart($startMonth,$startYear,$endMonth,$endYear,$curYear,$secondYear,$thirdYear,$title,$bgColor){
		if($endYear == ""){
			echo '<td title="'.$title.'" colspan="3" style="background-color:'.$bgColor.';">&nbsp;</td>
			<td colspan="12" style="width:24px;">&nbsp;</td>
			<td colspan="12" style="width:24px;">&nbsp;</td>
			<td colspan="12" style="width:24px;">&nbsp;</td>
			<td colspan="3" style="width:6px;">&nbsp;</td>';
		}else{
			$startMonth = ltrim($startMonth,"0");
			$endMonth = ltrim($endMonth,"0");
			$firstColsPan = 0;
			$secondColsPan = 0;
			$thirdColsPan = 0;
			$fourthColsPan = 0;
			...
			if($endYear < $startYear){
				...
				if($startYear < $curYear){
					if($endYear < $curYear){
						$firstColsPan = 3;
						echo '
							<td title="'.$title.'" style="background-color:'.$bgColor.';" colspan="'.$firstColsPan.'">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="3" style="width:6px;">&nbsp;</td>';
					}else if($endYear == $curYear){
						$firstColsPan = 3+$endMonth;
						$firstColsPanWidth = $firstColsPan*2;
						$secondColsPan = 12-$endMonth;
						$secondColsPanWidth= $secondColsPan*2;
						echo '
							<td colspan="'.$firstColsPan.'" title="'.$title.'" style="width:'.$firstColsPanWidth.'px;background-color:'.$bgColor.';">&nbsp;</td>
							<td colspan="'.$secondColsPan.'" style="width:'.$secondColsPanWidth.'px;">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="3" style="width:6px;">&nbsp;</td>';
					}else if($endYear == $secondYear){
						$firstColsPan = 3+12+$endMonth;
						$firstColsPanWidth = $firstColsPan*2;
						$secondColsPan = 12-$endMonth;
						$secondColsPanWidth= $secondColsPan*2;
						echo '
						...
						echo '
							<td colspan="42" title="'.$title.'" style="width:84px;background-color:'.$bgColor.';" >&nbsp;</td>';
					}
				}else if($startYear == $curYear){
					if($endYear == $curYear){
						$firstColsPan = $startMonth-1;
						$firstColsPanWidth = $firstColsPan*2;
						$secondColsPan = 12 - $firstColsPan;
						$secondColsPanWidth= $secondColsPan*2;
						echo '
							<td colspan="3">&nbsp;</td>
							<td colspan="'.$firstColsPan.'" style="width:'.$firstColsPanWidth.'px;" >&nbsp;</td>
							<td title="'.$title.'" colspan="'.$secondColsPan.'" style="width:'.$secondColsPanWidth.'px;background-color:'.$bgColor.';">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="3" style="width:6px;">&nbsp;</td>';
					}elseif($endYear == $secondYear){
						$firstColsPan = $startMonth-1;
						$firstColsPanWidth = $firstColsPan*2;
						$secondColsPan = 24 - $firstColsPan;
						$secondColsPanWidth= $secondColsPan*2;
						echo '
							<td colspan="3">&nbsp;</td>
							<td colspan="'.$firstColsPan.'" style="width:'.$firstColsPanWidth.'px;" >&nbsp;</td>
							...
							<td colspan="12" style="width:24px;">&nbsp;</td>
							<td colspan="3" style="width:6px;">&nbsp;</td>';
					}elseif($endYear == $thirdYear){
						$firstColsPan = $startMonth-1;
						$firstColsPanWidth = $firstColsPan*2;
						$secondColsPan = 36 - $firstColsPan;
						$secondColsPanWidth = $secondColsPan*2;
						echo '
							<td colspan="3">&nbsp;</td>
							<td colspan="'.$firstColsPan.'" style="width:'.$firstColsPanWidth.'px;" >&nbsp;</td>
						...
						echo '
							<td colspan="3">&nbsp;</td>
							<td title="'.$title.'" colspan="39" style="width:78px;background-color:'.$bgColor.';">&nbsp;</td>';
					}
					...
					<td colspan="12" style="width:24px;">&nbsp;</td>
					<td colspan="12" style="width:24px;">&nbsp;</td>
					<td title="'.$title.'" style="background-color:'.$bgColor.';" colspan="3">&nbsp;</td>
					';
				}
			}
		}
