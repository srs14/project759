...
class phpMyEdit
{
	...
	var $default_buttons = array(
			'L' => array('<<','<','add','>','>>','goto','goto_combo'),
			'F' => array('<<','<','add','>','>>','goto','goto_combo'),
...
//			'V' => array('cancel')
	...
	function list_operation()   { return ! $this->change_operation()
	...
	function set_values_from_table($field_num, $strict = false) /* {{{ */
	{
		...
		$res = mysql_query($ret);
		if($res === false) return softDie('Bad SQL query checking RET data in . Query:'.$query);
...
//						$this->display_delete_field1($row, $k);						
	...
	function display_button($name, $position = 'up') /* {{{ */
	{
		...
		if (in_array($name, array('add','view','change','copy'))) {
...
//				$query_real   = 'UPDATE '.$this->sd.$this->tb.$this->ed.' SET '.$this->sd.$fd.$this->ed.'='.$value;
				if(!isset($exists))
				{
					$query_tmp = 	'SELECT larvol_id  FROM data_manual ' . $where_part;
					$res = mysql_query($query_tmp);
					if ($res === false) return softDie('Bad SQL query getting data from data_manual');
