...
class phpMyEdit
{
	...
	function copy_operation()   { global $db;return $this->operation == $this->labels['Copy']   && $this->copy_enabled() && $db->loggedIn();   }
				...
				if ($select_recs) {
					...
					if ($this->nav_graphic_links()) {
						...
						if ($this->view_enabled()) {
							...
							echo 'view.png" height="15" width="16" border="0" ';
						...
						if ($this->change_enabled() and $db->loggedIn() and ($db->user->userlevel=='admin'|| $db->user->userlevel=='root' || $db->user->userlevel=='user') ) { // allow editing to user
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'jedit.png" height="15" width="16" border="0" ';
