...
{
	pr('<b> Calculating Investigators : '. $DC_id['id'] . '</b>');
...
//	pr($entity2ids);
	...
	foreach ($entity1ids as $ak=>$av)
	{
		...
		$Query = 'select parent from entity_relations where child ="' . $DC . '" ';
		$Res = mysql_query($Query);
		$diseaseids=array();
		while($row = mysql_fetch_assoc($Res))
		{
			$diseaseids[] = $row['parent'];
		}
		if($DC==$pv['id']) 
			$nonDC=$av['id'];
		else
...
function isDiseaseCategory($av,$pv)
{
	...
	if(!empty($av))
	{
		$query = 'SELECT id from entities where id="'.$av.'" and class="Investigator" limit 1';
		if(!$res = mysql_query($query))
		{
			$log='There seems to be a problem with the SQL Query:'.$query.' Error:' . mysql_error();
			$logger->error($log);
	...
	{
		$query = 'SELECT id from entities where id="'.$pv.'" and class="Investigator" limit 1';
		if(!$res = mysql_query($query))
		{
			$log='There seems to be a problem with the SQL Query:'.$query.' Error:' . mysql_error();
			$logger->error($log);
