...
if (!defined('KRUMO_DIR')) {
	define('KRUMO_DIR', 'krumo' . DIRECTORY_SEPARATOR);
	}
...
function urlPath()  //gives the userland path to this file all the way from http://
{
	...
	return substr($beforeq,0,strrpos($beforeq,'/')+1);
}
	...
	*/
	Private Static Function _css() {
		...
		if ($_css = $css != '') {
			...
			// the CSS (removed - put in header manually now)
			//

			// the JS (removed - put in header manually now)
			//
